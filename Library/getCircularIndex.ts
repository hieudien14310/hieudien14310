 ...
 * @returns
 */
function getCircularIndex(index: number, circular: number[]): number {
	const codeLength: number = circular.length
	let circularIndex: number = index % codeLength
	if (index === 0 || codeLength + circularIndex === codeLength) return 0
